          ...
          <Box sx={{ mb: 2 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
              {pod.members?.slice(0, 3).map((member) => (
                <Chip
                  key={member.id}
                  label={`${member.member?.name || 'Unknown'} (${(member.bandwidth_percentage * 100).toFixed(0)}%)`}
                  size="small"
                  variant="outlined"
                  sx={{ fontSize: '0.7rem' }}
                />
                ...
                {selectedPod.members?.map((member: any) => (
                  <ListItem key={member.id}>
                    ...
                    <ListItemText
                      ...
                      secondary={`${member.member?.team || 'Unknown Team'} • ${(member.bandwidth_percentage * 100).toFixed(0)}% bandwidth`}
                    />
                  </ListItem>
                ))}
              </List>
